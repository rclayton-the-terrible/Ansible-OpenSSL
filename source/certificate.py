...
TMPL_GEN_PK =   "openssl genrsa -out {}.key.pem {}"
...
TMPL_REVOKE = "openssl ca -config openssl.cnf -revoke {} -keyfile private/cakey.pem -cert cacert.pem"
...
class Certificate:
    ...
    def create_certificate(self):
        ...
        if not os.path.exists(self.hostname + ".key.pem"):
            ...
            changes.append("Created private key for {}.".format(self.hostname))
            changed = True

        if not os.path.exists(self.hostname + ".req.pem"):
            self.generate_certificate_request()
            changes.append("Created certificate request for {}".format(self.hostname))
            changed = True

        if not os.path.exists(self.hostname + ".cert.pem"):
            self.sign_certificate_request(target_path)
            changes.append("Signed certificate for {}".format(self.hostname))
            changed = True

        if not os.path.exists(self.hostname + ".keycert.pem"):
            self.create_key_cert_PEM()
            changes.append("Created key-cert PEM file for {}".format(self.hostname))

        if not os.path.exists(self.hostname + ".keycert.p12"):
            self.export_key_as_PKCS12()
            changes.append("Created PKCS12 version of the Private Key/Certificate Pair for {}".format(self.hostname))
            changed = True
    ...
    def revoke_certificate(self, cwd):
        os.chdir("..")

        cmd = TMPL_REVOKE.format(cwd + os.sep + self.hostname + ".cert.pem")
        self.execute_command(cmd)
    ...
    def remove_certificate(self):
        ...
        if not os.path.exists(target_path):
            return dict(success=True, changed=changed, changes=changes, msg="{} does not exist, therefore cert doesn't exist.".format(target_path))

        os.chdir(target_path)

        if os.path.exists(self.hostname + ".key.pem"):
            os.remove(self.hostname + ".key.pem")
            changes.append("Removed private key for {}.".format(self.hostname))
            changed = True

        if os.path.exists(self.hostname + ".req.pem"):
            os.remove(self.hostname + ".req.pem")
            changes.append("Removed certificate request for {}".format(self.hostname))
            changed = True

        if os.path.exists(self.hostname + ".cert.pem"):
            self.revoke_certificate(target_path)
            os.remove(self.hostname + ".cert.pem")
            changes.append("Removed certificate for {}".format(self.hostname))
            changed = True

        if os.path.exists(self.hostname + ".keycert.pem"):
            os.remove(self.hostname + ".keycert.pem")
            changes.append("Removed key-cert PEM file for {}".format(self.hostname))

        if os.path.exists(self.hostname + ".keycert.p12"):
            os.remove(self.hostname + ".keycert.p12")
            changes.append("Removed PKCS12 version of the Private Key/Certificate Pair for {}".format(self.hostname))
            changed = True
