...
class Certificate:
    ...
    def __init__(self, cadir, hostname, subj, p12password, isServerCert, subjectAltNames):
    ...
    def generate_private_key(self):
        cmd = TMPL_GEN_PK.format(self.get_target_path() + "/" + self.hostname, KEY_STRENGTH)
        self.execute_command(cmd)

    def generate_certificate_request(self):
        cmd = TMPL_GEN_REQ.format(self.get_target_path(), self.get_target_path() + "/" + self.hostname, self.subj)
        self.execute_command(cmd)
    ...
    def create_key_cert_PEM(self):
        keyPem = self.read_file(self.get_target_path() + "/" + self.hostname + ".key.pem")
        cerPem = self.read_file(self.get_target_path() + "/" + self.hostname + ".cert.pem.pub")
        with open(self.get_target_path() + "/" + self.hostname + ".keycert.pem", "w") as kcFile:
            kcFile.write(keyPem)
            kcFile.write("\n")
            kcFile.write(cerPem)
    ...
    def create_server_cnf(self, target_path):
        ...
        if not os.path.exists(fileOpensslCnf):
            ...
            alt_names = "DNS:" + self.hostname
            opensslCnf.write(OPENSSL_CNF.format(self.cadir, self.hostname, alt_names))
            opensslCnf.close()
            changed = True        
    ...
    def revoke_certificate(self):
        ...
        cmd = TMPL_REVOKE.format(self.cadir, CURDIR + os.sep + self.hostname + ".cert.pem.pub")
        self.execute_command(cmd)
    ...
    def remove_certificate(self):
        ...
        if os.path.exists(self.hostname + ".cert.pem.pub"):
            self.revoke_certificate()
            os.remove(self.hostname + ".cert.pem.pub")
